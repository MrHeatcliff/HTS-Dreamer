...
export WANDB_TAGS=v19_ab,locked_hier_x3,atari100k,size12m,alien_breakout,retry_no_video,no_policy_video
...
/mnt/disk1/backup_user/dat.tt2/xuance/.venv/bin/python -m dreamerv3.main_hts \
  ...
  --seed 2 \
  --logdir /mnt/disk1/backup_user/dat.tt2/xuance/logs/external_baselines/dreamerv3_official_hts_v19_ab/hts_locked_hier_x3/breakout/seed_2_retry_no_video \
  --run.steps 110000 \
  --run.envs 1 \
  --run.train_ratio 256 \
  --run.log_every 250 \
  ...
  --jax.prealloc False \
  --jax.jit True \
  2>&1 | tee /mnt/disk1/backup_user/dat.tt2/xuance/external_baselines/dreamerv3-official/paper_artifacts/atari_gate_d2_v19_ab/run_logs/v19_ab__hts_locked_hier_x3__breakout__seed2__retry_no_video.log
